...
public class ImmutableDictionaryBuilderTest extends ImmutableDictionaryBuilderTestBase {
//[Fact]
...
//}
    ...
    @Override
    protected <K, V> ImmutableHashMap<K, V> getEmptyImmutableMap() {
        return ImmutableHashMap.create();
    }
    ...
    @Override
    protected <K, V> K getKeyHelper(Map<K, V> dictionary, K equalKey) {
        return ((ImmutableHashMap.Builder<K, V>)dictionary).getKey(equalKey);
    }

    @Override
    protected <K, V> ImmutableMapBuilder<K, V> getBuilder(ImmutableMap<K, V> basis) {
        ImmutableHashMap<K, V> basisHashMap;
        if (basis != null) {
            basisHashMap = (ImmutableHashMap<K, V>)basis;
        } else {
            basisHashMap = getEmptyImmutableMap();
        }

        return basisHashMap.toBuilder();
    }
}
